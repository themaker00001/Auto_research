...
Usage:
    uv run train.py                  # default 5-minute budget
    uv run train.py --time-budget 600  # 10-minute budget
"""
...
class MLP(nn.Module):
    ...
    def __call__(self, x):
        x = self.c_fc(x)
        x = mx.maximum(x, 0) ** 2  # relu().square()
...
class GPT(nn.Module):
    ...
    def _get_masks(self, seq_len):
        ...
        return [self._mask_cache[(seq_len, ws)] for ws in self.window_sizes]
    ...
    def estimate_flops(self):
        ...
        for ws in self.window_sizes:
            effective_seq = t if ws < 0 else min(ws, t)
...
class AdamW:
    """AdamW optimizer with per-parameter-group learning rates.
    Classifies parameters by path to apply different LRs for
    embeddings, unembeddings, matrix weights, and scalars.
    """
...
# Optimization
TOTAL_BATCH_SIZE = 2**16 # ~65K tokens per optimizer step (reduced for Apple Silicon)
EMBEDDING_LR = 0.6      # learning rate for token embeddings (Adam)
UNEMBEDDING_LR = 0.004  # learning rate for lm_head (Adam)
MATRIX_LR = 0.04        # learning rate for matrix parameters (Adam)
SCALAR_LR = 0.5         # learning rate for per-layer scalars (Adam)
WEIGHT_DECAY = 0.2      # weight decay for matrix params
...
# Model size
DEPTH = 4               # number of transformer layers (smaller for Apple Silicon)
DEVICE_BATCH_SIZE = 16   # per-device batch size (reduce if OOM)
FINAL_EVAL_BATCH_SIZE = 16   # eval batch size (keep small on limited memory)
...
if __name__ == "__main__":
    ...
    # Load data first and time it separately
    ...
    while True:
        ...
        for _ in range(grad_accum_steps):
            ...
            mx.eval(loss, grads)  # force evaluation to prevent graph buildup
